# project-p 商户支付 OpenAPI · Go SDK

零第三方依赖的 Go SDK，覆盖 project-p 商户 OpenAPI 全部 11 个端点（代收 5 + 代付 6），含
...
2. 在自己的 `go.mod` 里 `require github.com/cniia/project-p-merchant-sdk-go v0.0.0` 后用 `replace`
   指向本地路径（私有未发布时）：

```go
import projectp "github.com/cniia/project-p-merchant-sdk-go"
```
...
func main() {
    client := projectp.NewClient(projectp.Config{
        ...
        Environment:  projectp.Sandbox,      // 或 projectp.Production
    })
    ...
    if err != nil {
        ...
        fmt.Println("传输失败:", err) // *projectp.TransportError
        return
    }
...
```go
// 预设：正式
projectp.NewClient(projectp.Config{Environment: projectp.Production, /* ... */})
// 预设：本地沙箱（http://127.0.0.1:3090/api/open/v1）
projectp.NewClient(projectp.Config{Environment: projectp.Sandbox, /* ... */})
// 自定义基址覆盖（代理专有域名或其它端口）——优先于 Environment
projectp.NewClient(projectp.Config{BaseURL: "https://api.<agent_domain>/api/open/v1", /* ... */})
```
...
```go
base := projectp.BuildSignBase(payload, secret) // 便于逐字节断言
sign := projectp.Sign(payload, secret)
ok := projectp.VerifyCallback(payload, secret)  // 时序安全
```

序列化口径与服务端 `web-shared/src/openapi/sign.ts` 逐字节一致：嵌套 JSON 自写 key 升序递归、
紧凑无空格、不转义 `/` 与非 ASCII 与 `<>&`、只转 `" \ \b\f\n\r\t` 及其余控制字符；顶层标量原样
（`bool→true/false`、整数十进制）。线缆序列化用 `Encoder.SetEscapeHTML(false)`。

## 错误处理

- 业务失败（信封 `code != 0`）→ `*projectp.APIError`（含 `Code`/`Message`/`Data`/`Raw`）。
- HTTP/网络/非 2xx/解析失败 → `*projectp.TransportError`（含 `StatusCode`/`Raw`/`Err`，可 `errors.Unwrap`）。
...
```go
var apiErr *projectp.APIError
var trErr  *projectp.TransportError
switch {
