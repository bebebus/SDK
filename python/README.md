# project-p 商户支付 OpenAPI — Python SDK

**零第三方依赖**的 Python 3 SDK：HTTP 用 `urllib.request`，签名用 `hmac`/`hashlib`，测试用标准库 `unittest`。无需 `pip install` 任何运行时依赖。

签名算法与服务端 `web-shared/src/openapi/sign.ts`、`project-p-test/sign.js` 逐字节一致，单测对 [`../test-vectors.json`](../test-vectors.json) 全量复现 `base` 与 `sign`。

## 引入（无需安装依赖）

把 `projectp_sdk/` 目录放进你的项目（或把本目录加入 `sys.path` / `PYTHONPATH`）即可：

```python
import sys
sys.path.insert(0, "/path/to/project-p-sdk/python")
...
```python
from projectp_sdk import Client, Config, Environment, ApiError, TransportError
...
正式真实地址常按上级代理专有域名派生（`https://api.<agent_domain>/api/open/v1`），用 `base_url=` 显式覆盖：

```python
config = Config(
    merchant_no="M00000001", api_key="ak_xxx",
    api_secret_pay="...", api_secret_payout="...",
    base_url="https://api.agent.example.com/api/open/v1",  # 覆盖预设
)
```
...
```python
from projectp_sdk import build_sign_base, sign, verify_callback
