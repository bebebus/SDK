...
- **Breaking (Python only)**: `pay_create` drops the `channel_code` keyword argument (it never existed in the v1 server contract); `pay_method` is now required and a missing `pay_method` raises `TypeError`, mirroring the existing `payout_create` v1 contract. Callers passing `channel_code=` will get a `TypeError: Client.pay_create() got an unexpected keyword argument 'channel_code'` and must switch to `pay_method`.
- **Breaking (Go only)**: the module path is now `github.com/bebebus/SDK/go/v2` (Go semantic import versioning — a v2+ module must carry a `/vN` path suffix, or the version cannot be resolved by `go get`/`go list` at all). Update `go get github.com/bebebus/SDK/go/v2@v2.0.0` and `import "github.com/bebebus/SDK/go/v2"`; the release tag itself stays `go/v2.0.0` (no repeated `/v2` segment, per Go's tag-mapping rule for subdirectory modules). The 1.x module path (`github.com/bebebus/SDK/go`, no `/v2`) and its tags remain resolvable for callers pinned to 1.x.
- **Docs (SIGNING/INTERFACES/README, zh/en)**: the v2 request-binding section, the "payout exception" callout, and every `channel_code` / `pay_methods` + `channel_codes` mention describing v2-only response shapes are removed. `pay/create` and `pay-methods/query` are documented as v1-only: `pay_method` required, `data.methods[]` response. The same cleanup was applied to source docblocks (`groupsQuery`/`GroupsQuery` in all five languages previously pointed callers at a `channel_codes` field that no longer exists anywhere in v1).
